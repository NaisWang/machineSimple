...
export default {
  ...
  methods: {
    initTableData() {
      oneSaleApi.getOneSale(this.searchMachine, 2).then(resp => {
        let data = resp.data;
        if (data["code"] === 200) {
    ...
    addPurchaseOrderReceiptMachinesByExcel(machineExcelInfo, ExcelTitleIndexNumber) {
      for (let i = 1; i < machineExcelInfo.length; i++) {
        ...
        purchaseOrderReceiptMainInfo["times"] = 2
        this.purchaseOrderReceiptMachines.push(purchaseOrderReceiptMainInfo);
      }
      oneSaleApi.putOneSale(this.purchaseOrderReceiptMachines, 2).then(resp => {
        this.initTableData()
      })
    },
